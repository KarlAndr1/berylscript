...
const char *lex_err_str(lex_token tok, size_t *str_len) {
	...
	switch(tok.content.err_type) {
		case TOK_ERR_NULL: ERR_MSG("Lexing error: Null");
		
		case TOK_ERR_MISSING_STR_END: ERR_MSG("Lexing error: Missing string terminator");
		
		case TOK_ERR_SIZE_OVERFLOW: ERR_MSG( "Lexing error: Symbol too long");
		
		case TOK_ERR_INT_OVERFLOW: ERR_MSG("Lexing error: Integer too large");
		
		default:ERR_MSG("Lexing error: None");
	}
}
#undef ERR_MSG
...
struct lex_token parse_number(struct lex_state *state) {
	...
	if(!is_at_end(state) && *state->at == '.') { //Decimals
		state->at++;
		i_float decimals = 0;
		i_float pow = 1.0;
		while(!is_at_end(state) && (is_digit(*state->at) || *state->at == '\'')) {
...
struct lex_token next_token(struct lex_state *state) {
	...
	if(sym_len > 0) {
		if(is_op_character(sym_start[sym_len - 1])) {
			sym_tok.type = TOK_OP;
			if(sym_len > 1) {
				if(sym_start[sym_len - 1] == ':') // Symbols ending in : are treated as operators, however the ':' is not included in the symbol
					sym_tok.content.sym.len--;
				else if(sym_start[sym_len - 1] == '=' && sym_start[0] != '=') { //Symbols ending in = that don't begin with = are treated as an assignment function, i.e x += 2
