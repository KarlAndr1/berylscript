...
static void beryl_print_i_val(void *f, i_val val) {
...
static i_val parse_eval_fn_assign(struct lex_state *lex, bool eval, struct lex_token var_tok,  struct lex_token fn_assign) {
	...
	//if(n_args == 0) {
	//	blame_token(lex, lex_peek(lex));
	//	err = BERYL_ERR("Expected arguments following function assignment");
	//	goto ERR;
	//}
				
	i_val res = beryl_call(fn, args_begin, n_args + 1);
	beryl_release(fn);
...
static i_val parse_eval_term(struct lex_state *lex, bool eval) {
	...
	switch(tok.type) {
		...
		case TOK_DOT: {
			struct lex_token sym = lex_pop(lex);
			if(sym.type != TOK_SYM && sym.type != TOK_OP) {
				blame_token(lex, sym);
				return BERYL_ERR("Expected symbol following '.'");
			}
			return BERYL_STATIC_STR(sym.content.sym.str, sym.content.sym.len);
		}
...
static i_val parse_eval_subexpr(struct lex_state *lex, bool eval) {
	...
	while( lex_accept(lex, TOK_OP, &op) ) {
		...
		i_val res = beryl_call(op_fn, args, 2);
		beryl_release(op_fn);
		beryl_release(next_term);
		beryl_release(term);
		term = res;
...
static i_val parse_eval_expr(struct lex_state *lex, bool eval, bool ignore_newlines) {
	...
	i_val res = beryl_call(fn, args_begin, n_args);
	beryl_release(fn);
	if(BERYL_TYPEOF(res) == TYPE_ERR)
		blame_token(lex, fn_tok);
...
i_val beryl_call(i_val fn, const i_val *args, size_t n_args) {
	if(n_args > I_SIZE_MAX)
		return BERYL_ERR("Too many arguments");
	...
	switch(fn.type) {
		...
		case TYPE_EXT_FN: {
			...
			if(ext_fn->arity < 0) {
				if(n_args < (size_t) -(ext_fn->arity + 1))
					return BERYL_ERR("Not enough arguments");
		...
		case TYPE_FN: {
			return call_internal_fn(fn, args, n_args);
		}
		
		case TYPE_TABLE: {
			if(n_args == 0)
				return BERYL_ERR("Cannot index table without key");
			if(n_args == 1)
				return index_table(fn, args[0]);
			...
			i_val res = beryl_call(member, args + 1, n_args - 1);
			beryl_release(member);
		...
		case TYPE_ARRAY: {
			if(n_args != 1)
				return BERYL_ERR("Can only index array with an index");
			
			if(BERYL_TYPEOF(args[0]) != TYPE_NUMBER) {
				beryl_blame_arg(args[0]);
				return BERYL_ERR("Can only index array with a number");
			}
			i_float f = beryl_as_num(args[0]);
			if(!beryl_is_integer(args[0]) || f < 0) {
				beryl_blame_arg(args[0]);
				return BERYL_ERR("Can only index array with a positive integer number");
			}
			i_size index = f;
			...
			return beryl_retain(beryl_get_raw_array(fn)[index]);
		}
		
		default:
			// Experimental feature: postfix calling. I.e x as-string is the same as as-string x if x is not callable 
			//if(n_args == 1 && (BERYL_TYPEOF(args[0]) == TYPE_FN || BERYL_TYPEOF(args[0]) == TYPE_EXT_FN)) {
			//	return beryl_call(args[0], &fn, 1);
			//}
			beryl_blame_arg(fn);
			return BERYL_ERR("Attempting to call non-function value");
	}
}

i_val beryl_pcall(i_val fn, const i_val *args, size_t n_args) {
	i_val res = beryl_call(fn, args, n_args);
	clear_err();
	return res;
}

i_val beryl_eval(const char *src, size_t src_len, bool new_scope, enum beryl_err_action err) {
	struct lex_state lex;
